...
def calculate_health_score(df, temp_cols, vib_cols, current_cols):
    ...
    for idx in range(len(df)):
        ...
        # Temperature penalty
        if temp_cols:
            temp_values = df[temp_cols].iloc[idx]
            ...
            if temp_max > temp_threshold_critical:
                score -= 30
            elif temp_max > temp_threshold_warn:
                score -= 15
        
        # Vibration penalty
        if vib_cols:
            vib_values = df[vib_cols].iloc[idx]
            vib_max = vib_values.max()
            vib_threshold_warn = df[vib_cols].quantile(0.75).max()
            vib_threshold_critical = df[vib_cols].quantile(0.90).max()
            
            if vib_max > vib_threshold_critical:
                score -= 30
            elif vib_max > vib_threshold_warn:
                score -= 15
        
        # Current penalty
        if current_cols:
            current_values = df[current_cols].iloc[idx]
            ...
            if current_max > current_threshold_critical:
                score -= 20
            elif current_max > current_threshold_warn:
                score -= 10
        
        health_scores.append(max(0, score))  # Ensure non-negative
...
def create_failure_labels(df, machine_id):
    ...
    if temp_cols:
        temp_threshold = df[temp_cols].quantile(0.90).max()
    ...
    if vib_cols:
        vib_threshold = df[vib_cols].quantile(0.90).max()
        vib_high = df[vib_cols].max(axis=1) > vib_threshold
        failure_score += vib_high.astype(int)
    
    # Current threshold
    current_cols = [col for col in df.columns if 'current' in col.lower()]
    if current_cols:
        current_threshold = df[current_cols].quantile(0.90).max()
        current_high = df[current_cols].max(axis=1) > current_threshold
